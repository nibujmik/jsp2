...
<script type="text/javascript">
	function fileValid(){
		const imgfile = document.forms[0].pic //document.getElementById('imgfile')도 가능
		const maxSize = 1*1024*1024;
		
		//let isvaild = true;
		//button 타입은 submit으로 하고 form태그의 onsubmit 속성에 유효성 검사 함수를 설정할 때는 isVaild 변수 필요 없음.
		//console.log(document.forms[0].pic.value)
		if(imgfile.value==""|| imgfile.value==null){
			alert('파일 첨부는 필수입니다.')
			//isVaild=false;
			return false;	//onsubnit 속성으로 호출된 함수에서 return false이면 submit안합니다.
		}else{
...
<h3>이미지 파일 업로드 하기</h3>
<hr>
	<!-- 파일 업로드를 할 때는 반드시 method="post" enctype="multipart/form-data" -->
	<form action="gallerySave.jsp" method="post" enctype="multipart/form-data" onsubmit="return fileValid()">
		...
		<!-- 웹에서는 파일의 형식을 확장자로만 구분하지 않고 MIME형식을 사용합니다.
			image/*, image/*.png, image/*.jpeg, video/*등등
			acceot 속성은 파일선택 창에서 초기 형식. -> *.*모든 파일로 변경 및 파일 전송도 가능 -->
		<button>전송</button>
	</form>
	<!-- 
		...
		2. 서버 페이지에서는 file 타입 요소로 전달된 파일을 전송받습니다.(업로드)
				ㄴ서버에서 업로드 폴더 설정이 필요 
		3. 2번에서 파일 전송에 필요한 Request 처리가 별도로 필요합니다. -> MultipartRequest필요
				ㄴdependency 추가
	 -->
</body>
</html>